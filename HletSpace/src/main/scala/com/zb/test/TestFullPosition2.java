...
public class TestFullPosition2 {
...
//        List list2 = Arrays.asList(2, 3, 6, 18, 26);
        ...
        // 现在需要解决 前一个点和后一个点  的比较   需要定位当前点属性
        for (int i = 0; i < dtos.size() - 1; i++) {
            //    System.out.println(dtos.get(i).getLabel() + "---" + dtos.get(i + 1).getLabel() + "==" + (dtos.get(i + 1).getNum() - dtos.get(i).getNum()));
            int lagNum = dtos.get(i + 1).getNum() - dtos.get(i).getNum();
            if (lagNum == 1) {
                continueQueue.add(dtos.get(i));
                // continue;
            } else if ((lagNum > 1 && lagNum < 10) && count < 6) {
                continueQueue.add(dtos.get(i));
                count++;
                //   continue;
            } else {
                disContinueQueue.add(dtos.get(i));
                System.out.println("不连续轨迹："+dtos.get(i));
